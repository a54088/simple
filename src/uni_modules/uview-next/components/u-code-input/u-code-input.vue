...
<script>
	 ...
	 * @property {String | Number}	type			    输入框类型 （默认 'number' ）
				...
				return index => {
					...
					if (this.mode === 'box') {
						...
						if(this.borderColor != 'none'){
							style.border = addUnit(this.hairline ? 0.5 : 1) + ' solid ' + this.borderColor
						}
						...
						if (uni.$u.getPx(this.space) === 0) {
							// 给第一和最后一个盒子设置圆角
							if (index === 0) {
								style.borderTopLeftRadius = addUnit(this.round)
								style.borderBottomLeftRadius = addUnit(this.round)
							}
							if (index === this.codeLength.length - 1) {
								style.borderTopRightRadius = addUnit(this.round)
								style.borderBottomRightRadius = addUnit(this.round)
							}
							// 最后一个盒子的右边框需要保留
							if (index !== this.codeLength.length - 1) {
							...
							style.borderRadius = addUnit(this.round)
						}
					}
					if (index !== this.codeLength.length - 1) {
