<template>
	...
	>
		<view
			class="u-notify"
			:class="[`u-notify--${config.type}`]"
			:style="[backgroundColor, $u.addStyle(customStyle)]"
		>
			<u-status-bar v-if="config.safeAreaInsetTop"></u-status-bar>
			<view class="u-notify__warpper">
				<slot name="icon">
					<u-icon
						v-if="['success', 'warning', 'error'].includes(config.type)"
						:name="icon"
						:color="config.color"
						:size="1.3 * config.fontSize"
				...
				<text
					...
					:style="[{
						fontSize: $u.addUnit(config.fontSize),
						color: config.color
					}]"
				>{{ config.message }}</text>
			</view>
	...
	export default {
		...
		computed: {
			containerStyle() {
				...
				const style = {
					top: uni.$u.addUnit(this.config.top === 0 ? top : this.config.top),
			...
			show(options) {
				...
				if (this.config.duration > 0) {
					this.timer = setTimeout(() => {
						...
						typeof(this.config.complete) === 'function' && this.config.complete()
