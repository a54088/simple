...
<script>
	 ...
	 * @property {Function}			select			选中时触发的事件 ( 默认 () => {} )
	 * */ 
	export default {
		name: 'u-index-list',
		...
		methods: {
			init() {
				...
				this.scrollViewHeight = this.windowInfo.windowHeight - uni.$u.getPx(this.customNavHeight)
			},
			// 索引列表被触摸
			touchStart(e) {
				// 获取触摸点信息
			...
			setIndexListLetterInfo() {
				this.getIndexListLetterRect().then(size => {
					...
					if (this.customNavHeight == 0) {
						// #ifdef H5
						customNavHeight = windowInfo.windowTop
						// #endif
						// #ifndef H5
