 ...
 * u-picker
 * @description 选择器
 * @property {Array}            v-model			    当前选中项对应的值	
 * @property {Array}            value			    当前选中项对应的值	
...
export default {
	...
	// #ifdef VUE3
	emits: ["update:modelValue", "confirm", "close", "cancel", "change"],
	// #endif
	methods: {
		openPicker() {
			this.isTrigger = true
		...
		handleValueChange(newValue) {
			...
			if (!newValue || !this.innerColumns.length) return
			...
			// 遍历每一列，找到匹配的值的索引
			const newIndex = this.innerColumns.map((column, columnIndex) => {
				const value = newValue[columnIndex]
				if (!column || !column.length) return 0
			...
			// 如果所有列都找到了对应的索引，则更新选中值
			if (newIndex.every(index => index !== -1)) {
				this.setIndexs(newIndex, true)
			}
		}
	},
}
