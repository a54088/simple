<template>
    <view class="u-textarea" :class="textareaClass" :style="[textareaStyle]">
        ...
        ></textarea>
		<!-- #ifndef MP-ALIPAY -->
        <text class="u-textarea__count" v-if="count">{{ maxlength > 0 ? innerValue.length + '/' + maxlength : innerValue.length }}</text>
		<!-- #endif -->
    </view>
</template>
 ...
 * @tutorial https://uview.d3u.cn/components/textarea.html
 *
 ...
 * @property {String}				borderRadius			边框圆角
...
export default {
    ...
    computed: {
        ...
        textareaClass() {
            let classes = [];
            this.border === "surround" && classes.push("u-border");
            this.border === "bottom" && classes.push("u-border-bottom");
            return classes.join(" ");
        },
        // 组件的样式
        textareaStyle() { 
    ...
    // #ifdef VUE3
	emits: ['update:modelValue', 'focus', 'blur', 'confirm', 'change', 'linechange', 'keyboardheightchange'],
	// #endif
    methods: {
        init(newVal){
