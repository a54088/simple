	...
	>
		<view
			...
			:class="[
				...
				innerCurrent === 0 && mode === 'subsection' && `u-subsection__bar--first--${shape}`,
				innerCurrent > 0 && innerCurrent < list.length - 1 && mode === 'subsection' && `u-subsection__bar--center`,
				innerCurrent === list.length - 1 && mode === 'subsection' && `u-subsection__bar--last--${shape}`
			]"
		></view>
		<view
			...
			:key="index"
		>
			<text class="u-subsection__item__text" :style="[textStyle(index,item)]">{{ getText(item) }}</text>
		</view>
	</view>
</template>
 ...
 * @property {String}			disabledBgColor	禁用时的背景颜色（默认 '#e5e5e5' ）
 * @property {String}			disabledColor	禁用时的文字颜色
 *
 *
...
export default {
	...
	computed: {
		wrapperStyle() {
			...
			// button模式时，设置背景色
			if (this.mode === 'button') {
				style.backgroundColor = this.disabled ? this.disabledBgColor : this.bgColor;
			}
		...
		barStyle() {
			...
			if (this.mode === 'subsection') {
				...
				style.backgroundColor = this.disabled ? this.disabledBgColor : this.activeColor;
			}

			if (this.mode === 'button' && this.barColor) {
				style.backgroundColor = this.disabled ? this.disabledBgColor : this.barColor;
			}
		...
		itemStyle(item) {
			return (item) => {
				...
				if (this.mode === 'subsection') {
					// 设置border的样式
					style.borderColor = this.disabled || (typeof item === 'object' && item.disabled) ? this.disabledColor : this.activeColor;
		...
		textStyle(index, item) {
			return (index, item) => {
				...
				let inactiveColor = this.inactiveColor;
				let activeColor = this.activeColor;
				let disabledColor = this.disabledColor;
				...
				if (this.mode === 'subsection') {
					if (isDisabled) {
						...
						style.color = this.innerCurrent === index ? '#fff' : inactiveColor;
					}
				} else {
					...
					if (isDisabled) {
						...
						style.color = this.innerCurrent === index ? activeColor : inactiveColor;
					}
				}
				return style;
			};
