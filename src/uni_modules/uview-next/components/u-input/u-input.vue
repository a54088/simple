...
export default {
	...
	computed: {
		...
		inputClass() {
			...
			border === "bottom" && (classes = classes.concat(["u-border-bottom"]));
			return classes.join(" ");
		},
		// 组件的样式
		wrapperStyle() {
			...
			if(this.border !== "bottom"){
                style.borderRadius = uni.$u.addUnit(this.shape == "circle" ? "100px" : this.round)
            }
			...
			// 无边框时，去除内边距
			if (this.border === "none" && this.backgroundColor == '') {
				style.padding = "0";
			} else {
