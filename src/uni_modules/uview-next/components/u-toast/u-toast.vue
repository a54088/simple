	...
	<view class="u-toast">
		...
		>
			...
			>
				...
				></u-icon>
				<u-gap
					v-if="tmpConfig.title && (tmpConfig.type === 'loading' || tmpConfig.loading)"
					height="12"
					bgColor="transparent"
				></u-gap>
				<text
					v-if="tmpConfig.title"
					class="u-toast__content__text"
					:class="['u-toast__content__text--' + tmpConfig.type]"
					style="max-width: 400rpx;"
				>{{ tmpConfig.title }}</text>
			</view>
		</u-overlay>
	</view>
</template>
...
<script>
	 ...
	 * @property {String | Number}	title		显示的文字内容
	 * @property {String}			icon		图标，或者绝对路径的图片
	 * @property {String}			type		主题类型 （默认 default）
	 * @property {Boolean}			mask		是否显示透明遮罩，防止点击穿透 （默认 false ）
	 * @property {String}			position	位置 （默认 'center' ）
	 * @property {String | Number}  duration	展示时间，单位ms （默认 2000 ）
	 * @property {Function}			success	    执行完后的回调函数
	 * @property {Object}			customStyle	组件的样式，对象形式
	 * @event {Function} open 显示toast，如需一进入页面就显示toast，请在onReady生命周期调用
	 * @example <u-toast ref="uToast" />
	 */
	import mpMixin from '../../libs/mixin/mpMixin'
	import mixin from '../../libs/mixin/mixin'
	import { setToastRef } from '../../libs/function/toast.js'
	export default {
		name: 'u-toast',
		...
		data() {
			return {
				...
				config: {
					title: '', // 显示文本
					type: 'black', // 主题类型，primary，success，error，warning，black
		...
		computed: {
			...
			loadingIconColor() {
				...
				if (['error', 'warning', 'success', 'primary'].includes(this.tmpConfig.type)) {
					...
					// 必须为rgb格式的，所以这里做一个处理
					color = uni.$u.hexToRgb(uni.$u.theme[this.tmpConfig.type])
				}
				return color
			},
			// 内容盒子的样式
			contentStyle() {
				const windowHeight = uni.$u.window().windowHeight, style = {}
				let value = 0
		...
		methods: {
			// 显示toast组件，由父组件通过this.$refs.xxx.open(options)形式调用
			open(options) {
			...
			// 隐藏toast组件，由父组件通过this.$refs.xxx.close()形式调用
			close() {
				this.clearTimer()
			},
			clearTimer() {
...
<style lang="scss" scoped>
	...
	$u-toast-loading-border-padding: 20px 30px !default;
	$u-toast-loading-background-color:rgba(17, 17, 17, 0.7) !default;
	...
	.u-toast {
		&__content {
			...
			&--loading {
				...
				background-color: $u-toast-loading-background-color;
			}
