...
<script>
	 ...
	 * @property {String}			inputBgColor	输入框背景颜色 （默认 '#EBECEE' ）
	 * @property {String | Number}	buttonRound	    加减按钮圆角，单位px （默认 0 ）
	 * @property {String | Number}	iconSize		加减按钮图标大小，单位px （默认 15 ）
	 * 
	...
	export default {
			...
			buttonStyle() {
				return (type) => {
					const style = {
						...
						width: uni.$u.addUnit(this.buttonSize),
						borderRadius: uni.$u.addUnit(this.buttonRound),
						color: this.color
					}
		...
		// #ifdef VUE3
		emits: ['update:modelValue', 'focus', 'blur', 'change', 'plus', 'minus','overlimit'],
		// #endif
		methods: {
			init() {
				// #ifdef VUE2
