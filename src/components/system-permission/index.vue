<template>
  <view
    v-if="showPopup"
    class="uni-popup"
    :style="' top: ' + isNativeHead ? '' : StatusBar + ';'"
  >
    <view
...
export default {
    ...
    return {
      ...
      authList: {
        WRITE_EXTERNAL_STORAGE: {
          title: "啊圈对存储空间/照片权限申请说明",
          name: "设备储存",
          content:
            "便于您使用该功能上传您的照片/图片/视频及用于更换头像、意见反馈、保存相册、发布商品/分享、下载与客服沟通等场景中读取和写入相册和文件内容。",
        },
        ACCESS_FINE_LOCATION: {
          title: "啊圈对地理位置权限申请说明",
          name: "设备定位",
          content:
            "便于应用程序可以提供基于位置的服务、定位导航、附近搜索等功能。",
        },
        CAMERA: {
          title: "啊圈对相机/摄像头权限申请说明",
          name: "相机/摄像头",
          content:
            "便于您使用该功能拍照上传您的照片/视频及用于更换头像、意见反馈、保存相册、发布商品/动态、下载与客服沟通等场景中使用",
        },
        RECORD_AUDIO: {
          title: "啊圈对麦克风权限申请说明",
          name: "麦克风",
          content:
            "便于您使用该功能进行录音、语音通话、发布语音、与客服语音沟通等场景中使用",
        },
        CALL_PHONE: {
          title: "啊圈对拨打/管理电话权限申请说明",
          name: "拨打/管理电话",
          content:
            "便于您使用该功能联系买家、骑手或者客服、业务经理与联系等场景下使用",
        },
      },
    };
  },
  ...
  methods: {
    ...
    close(type) {
      this.ani = "";
      this.$nextTick(() => {
        let setTimeoutHandler = setTimeout(() => {
        ...
        if (plus.os.name == "Android") {
          ...
          plus.android.requestPermissions([_permissionID], (e) => {
            ...
            if (e.deniedAlways.length > 0) {
              ...
              uni.showModal({
                title: "温馨提示",
                content: `还没有${_this.authList[permissionID].name}权限，立即去设置开启？`,
                cancelText: "取消",
                confirmText: "去设置",
                showCancel: true,
                confirmColor: "#000",
